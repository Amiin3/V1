<template>
  <div class="min-h-screen bg-gray-50">
    <div class="bg-blue-600 text-white px-4 py-4 flex items-center space-x-3">
      ...
      <div class="flex-1">
        <h1 class="text-lg font-bold">Store Packages</h1>
      </div>
    ...
    <div class="max-w-2xl mx-auto p-4 space-y-3">
      ...
      <div v-if="packages.length === 0 && !error" class="text-center bg-white rounded-2xl shadow p-6 text-gray-500">Tidak ada paket.</div>

      <div v-for="pkg in packages" :key="pkg.package_option_code" class="bg-white rounded-2xl shadow p-4">
        <p class="font-bold text-gray-800">{{ pkg.name }}</p>
        <p class="text-sm text-gray-600">Rp {{ pkg.price?.toLocaleString('id-ID') }} • {{ pkg.validity }}</p>
        <p class="text-xs text-gray-400">{{ pkg.package_option_code }}</p>
      </div>
    </div>
  </div>
</template>
...
const toggleEnterprise = () => {
  router.get('/store/packages', { enterprise: enterprise.value ? '1' : '0' }, { preserveState: true, replace: true });
};
</script>
