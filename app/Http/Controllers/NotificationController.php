...
class NotificationController extends Controller
{
    ...
    public function index()
    {
        ...
        $accessToken = $activeUser['tokens']['access_token'];
        $notifications = [];
        $unreadCount = 0;
        $error = null;

        try {
            // dashboard_segments untuk ambil notifikasi
            ...
            if (isset($res['data']['notification']['data'])) {
    ...
    public function readAll()
    {
        ...
        try {
            ...
            foreach ($notifications as $n) {
                ...
                if ($notificationId) {
                    // Panggil get_notification_detail untuk menandai dibaca
                    $detailPath = "api/v8/notification/detail";
                    $detailPayload = [
                        "is_enterprise" => false,
                        "lang" => "en",
                        "notification_id" => $notificationId
                    ];
                    $this->sendRequest($detailPath, $detailPayload, $idToken);
                    $readCount++;
                }
            }

            return response()->json(['status' => 'SUCCESS', 'read_count' => $readCount]);
        } catch (\Exception $e) {
